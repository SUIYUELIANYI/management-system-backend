...
type GetFormByAddressLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetFormByAddressLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetFormByAddressLogic {
	return &GetFormByAddressLogic{
...
func (l *GetFormByAddressLogic) GetFormByAddress(req *types.GetByAddressReq) (resp *types.FormsResp, err error) {
	// 仅限区域负责人（42）和组织管理委员会（43，44）可以调用
	// 相比getAllForm接口多一个省份的匹配，由于只匹配省份，直接匹配前两个字符就行
	...
	// 取地址的前两个字符
	// rune 类型主要用于处理和操作 Unicode 字符串。通过将字符串转换为 []rune 切片，可以访问和操作字符串中每个字符（码点）而不仅仅是字节。
	address := string([]rune(req.Address)[:2])

	// forms, err := l.svcCtx.ApplicationFormModel.FindAllByAddressWithPage(l.ctx, l.svcCtx.ApplicationFormModel.RowBuilder(), req.Page, req.PageSize, address)
	// if err != nil {
	// 	return nil, response.Error(500, err.Error())
	// }
	...
	if userInfo.Role == 42 { // 区域负责人负责 0-待审批的申请表
		forms, err := l.svcCtx.ApplicationFormModel.FindAllByAddressAndStatusWithPage(l.ctx, l.svcCtx.ApplicationFormModel.RowBuilder(), req.Page, req.PageSize, address, 0)
		if err != nil {
			return nil, response.Error(500, err.Error())
		}
		if len(forms) > 0 {
		...
		forms, err := l.svcCtx.ApplicationFormModel.FindAllByAddressAndStatusWithPage(l.ctx, l.svcCtx.ApplicationFormModel.RowBuilder(), req.Page, req.PageSize, address, 1)
		if err != nil {
			return nil, response.Error(500, err.Error())
		}
		if len(forms) > 0 {
