...
func (m *defaultUserAuthModel) Insert(ctx context.Context, data *UserAuth) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.DeleteTime, data.DelState, data.UserId, data.AuthKey, data.AuthType)
	return ret, err
}
...
func (m *defaultUserAuthModel) Update(ctx context.Context, newData *UserAuth) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.UserId, newData.AuthKey, newData.AuthType, newData.Id)
	return err
}
