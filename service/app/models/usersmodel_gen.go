...
type (
	...
	Users struct {
		...
		Role       int64     `db:"role"`     // 身份 0-申请队员 1-岗前培训 2-见习队员 3-正式队员 4-督导老师 5-区域负责人 6-组委会 7-主任
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.DeleteTime, data.OpenId, data.Mobile, data.Username, data.Password, data.Sex, data.Email, data.Role, data.Avatar, data.Address, data.Birthday)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.DeleteTime, newData.OpenId, newData.Mobile, newData.Username, newData.Password, newData.Sex, newData.Email, newData.Role, newData.Avatar, newData.Address, newData.Birthday, newData.Id)
	return err
}
